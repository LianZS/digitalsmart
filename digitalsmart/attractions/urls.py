...
comment=Comment()
urlpatterns = {
    path("api/getCitysByProvince", areainfo.citylist),  # 获取省份下所有城市列表
    path("api/getRegionsByCity", areainfo.scencelist),  # 2.获取城市下所有地区列表
    path("api/getLocation_pn_percent_new", flow.scenceflow_data),  # 实时人流接口
    path("api/getLocation_trend_percent_new", flow.scenceflow_trend),  # 实时人流趋势
    path("api/getLocation_search_rate", comment.search_heat),  # 地区全网搜索次数
    path("api/getLocation_distribution_rate", flow.scence_people_distribution),  # 地区实时人口分布热力图数据
